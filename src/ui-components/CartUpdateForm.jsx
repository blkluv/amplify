...
export default function CartUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = cartRecord
      ? { ...initialValues, ...cartRecord, Products: linkedProducts }
      : initialValues;
  ...
  React.useEffect(resetStateValues, [cartRecord, linkedProducts]);
        ...
        try {
          ...
          productsToUnLink.forEach((original) => {
            if (!canUnlinkProducts) {
              throw Error(
                `Product ${original.id} cannot be unlinked from Cart because cartID is a required field.`
              );
            }
            promises.push(
              DataStore.save(
